...
router.get('/:id/download', async (req, res) => {
  try {
    const [rows] = await execute('SELECT * FROM documents WHERE id = ?', [parseInt(req.params.id)]);
    ...
    await execute('UPDATE documents SET download_count = download_count + 1 WHERE id = ?', [doc.id]);
...
router.delete('/:id', authenticate, async (req, res) => {
  try {
    ...
    await execute('DELETE FROM documents WHERE id = ?', [parseInt(req.params.id)]);
