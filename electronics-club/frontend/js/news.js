...
async function viewNews(id) {
  try {
    ...
    document.getElementById('view-author').textContent  = `<i class="fa-solid fa-pen-nib"></i> ${n.author_name}`;
    document.getElementById('view-date').textContent    = `<i class="fa-regular fa-calendar-days"></i> ${formatDateTime(n.created_at)}`;
    document.getElementById('view-views').textContent   = `<i class="fa-regular fa-eye"></i> ${(n.view_count||0)+1} ครั้ง`;
...
async function saveNews() {
  ...
  try {
    ...
    saveBtn.disabled = false; saveBtn.textContent = '<i class="fa-solid fa-floppy-disk"></i> บันทึกข่าว';
  }
}
